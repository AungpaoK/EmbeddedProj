#!/usr/bin/env bash
# Start the delivery stack, local POS controller, and Pi display kiosk in tmux.
set -eo pipefail
...
usage() {
    cat <<EOF
Usage: bash start_robot.sh [start|status|attach|stop]

  start   Start LiDAR, SLAM bridge, SLAM Toolbox, POS, and the Pi kiosk.
  status  Show tmux windows and check the local POS health endpoint.
  attach  Attach to the persistent tmux session.
  stop    Stop POS, kiosk, bridge, and the remaining ROS processes.
...
case "$ACTION" in
    ...
    stop)
        ...
        for window in pos kiosk bridge slam lidar; do
...
python3 -c 'import rclpy, serial' >/dev/null 2>&1 || {
    echo "Python packages rclpy and pySerial are required in the sourced ROS environment." >&2
    exit 1
}
for package in sllidar_ros2 slam_toolbox; do
...
bridge_command="${ROS_PREFIX} && cd ${SRC_Q} && MOTION_PORT=${MOTION_Q} MOTION_SERIAL_BAUD=${BAUD_Q} INVERT_ODOM_YAW=0 ODOM_TRACK_WIDTH_FACTOR=1.185 LIDAR_OFFSET_X=0.15 LIDAR_OFFSET_Y=0.0 LIDAR_YAW_OFFSET=${YAW_Q} SELF_FILTER_RADIUS=0.195 exec python3 slam_bridge.py"
slam_command="${ROS_PREFIX} && cd ${SRC_Q} && exec ros2 launch slam_toolbox online_async_launch.py slam_params_file:=./slam_toolbox_config.yaml"
