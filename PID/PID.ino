...
const float WHEEL_RADIUS = 0.065;         // 6.5 cm radius in meters
const float TICKS_PER_REV = 1920.0;       // Encoder pulses per wheel revolution
const float METERS_PER_PULSE = (2.0 * 3.14159265 * WHEEL_RADIUS) / TICKS_PER_REV;

// --- User Turn Equation Formula Variables ---
const float W = 0.35;                            // Wheelbase track width in meters
const float D = 3.14159265;                      // Pi (180 degrees in radians)
const float TPR = TICKS_PER_REV;                 // 1920 Ticks Per Revolution
const float MOVE_DISTANCE_METERS = 2.0;          // Straight target (2 Meters)

// --- Target Formula: N = (W / 2) * D * (TPR / (2 * PI * R)) ---
const float TARGET_TURN_TICKS = (W / 2.0) * D * (TPR / (2.0 * 3.14159265 * WHEEL_RADIUS));
...
void loop() {
  ...
  if (now - lastDebugPrintMS >= DEBUG_PRINT_INTERVAL_MS) {
    ...
    Serial.print("Phase:");
    Serial.print(runPhase);
    Serial.print(" Turn_Target_N:");
    Serial.println(TARGET_TURN_TICKS);
  }
}