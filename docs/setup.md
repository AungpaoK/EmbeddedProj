...
- Raspberry Pi 4 ที่เชื่อมต่อกับ Arduino Uno R3 หนึ่งบอร์ด, RPLIDAR และจอสัมผัส POS/Kiosk
- Arduino Uno ควบคุมมอเตอร์/Encoder/LED Matrix และอ่าน Keypad 4x4 ผ่าน PCF8574 ที่ A4 (SDA), A5 (SCL)
- จอสัมผัส Aprotii ขนาด 7 นิ้วต่อภาพด้วย Micro HDMI และรับข้อมูล touch ผ่าน USB; ไม่มี LCD ตัวอักษรแยก
...
พิกัดเริ่มต้นของระบบกำหนดครัวเป็น `(0, 0)` หันหน้าไปทาง `+X`, ทางแยกห่าง `2.0 m`, โต๊ะ 1 อยู่ `y=+0.6 m` และโต๊ะ 2 อยู่ `y=-0.6 m` ปรับ `JUNCTION_X`, `TABLE1_Y` และ `TABLE2_Y` ใน `.env` ให้ตรงกับพื้นที่จริงก่อนใช้งาน [ค่าพิกัดและพอร์ต](../src/config.py)
...
สคริปต์ตรวจพอร์ตและคอมไพล์ก่อนอัปโหลด จากนั้นให้พิมพ์ `FLASH` เพื่อยืนยัน ตรวจสอบว่าพอร์ตที่เลือกเป็น Arduino มอเตอร์ และยกล้อพ้นพื้นหรือถอดไฟมอเตอร์ไดรเวอร์ในการทดสอบครั้งแรก [สคริปต์แฟลช firmware](../flash_motion.sh)
...
| --- | --- | --- |
| รอรับงาน (`IDLE`/`COMPLETED`) | `A` / `B` | A เลือกชั้น 2; B เลือกชั้น 1 |
| รอรับงาน | `1` / `2` | เลือกโต๊ะ 1 / 2 ให้ชั้นที่เลือก; กดโต๊ะเดิมซ้ำเพื่อยกเลิก |
| รอรับงาน | `C` | สลับสถานะยืนยันอาหารของชั้นที่เลือก |
...
หมายเหตุการยืนยันอาหาร: ใน POS Server ปัจจุบัน การกำหนดโต๊ะใหม่ตั้งสถานะอาหารของชั้นนั้นเป็นยืนยันแล้ว ส่วนปุ่ม `C` ใช้สลับสถานะนี้ ให้ตรวจสถานะที่แสดงบน POS/Kiosk ก่อนกดเริ่มงาน
