...
tmux send-keys -t "$SESSION:0.1" \
  "source /opt/ros/jazzy/setup.bash && cd ~/EmbeddedProj/src && sleep 2 && echo '=== [2] Starting SLAM Bridge ===' && MOTION_PORT=/dev/ttyACM0 INVERT_LINEAR=1 INVERT_STEER=1 INVERT_ODOM_YAW=0 ODOM_TRACK_WIDTH_FACTOR=1.185 LIDAR_OFFSET_X=0.15 LIDAR_OFFSET_Y=0.0 LIDAR_YAW_OFFSET=180 SELF_FILTER_RADIUS=0.195 python3 slam_bridge.py" C-m
...
tmux send-keys -t "$SESSION:0.3" \
  "source /opt/ros/jazzy/setup.bash && cd ~/EmbeddedProj/src && sleep 6 && echo '=== [4] Starting Auto Explorer ===' && LIDAR_YAW_OFFSET=180 SCAN_TOPIC=/scan_filtered DRIVE_START_SPEED=0.25 DRIVE_MAX_SPEED=0.45 DRIVE_SPEED_STEP=0.05 DRIVE_FEEDBACK_INTERVAL=0.35 DRIVE_MIN_ENCODER_SPEED=0.08 ESCAPE_MIN_TURN_SPEED=1.20 ESCAPE_MAX_TURN_SPEED=2.40 ESCAPE_SPEED_STEP=0.30 ESCAPE_FEEDBACK_INTERVAL=0.35 ESCAPE_MIN_YAW_RATE=0.60 ESCAPE_DURATION=2.5 python3 auto_explorer.py" C-m
