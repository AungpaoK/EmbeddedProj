...
tmux send-keys -t "$SESSION:0.1" \
  "source /opt/ros/jazzy/setup.bash && cd ~/EmbeddedProj/src && sleep 2 && echo '=== [2] Starting SLAM Bridge ===' && MOTION_PORT=/dev/ttyACM0 INVERT_LINEAR=1 INVERT_STEER=1 INVERT_ODOM_YAW=0 ODOM_TRACK_WIDTH_FACTOR=1.185 LIDAR_OFFSET_X=0.15 LIDAR_OFFSET_Y=0.0 SELF_FILTER_RADIUS=0.195 python3 slam_bridge.py" C-m
...
tmux send-keys -t "$SESSION:0.3" \
  "source /opt/ros/jazzy/setup.bash && cd ~/EmbeddedProj/src && sleep 6 && echo '=== [4] Starting Auto Explorer ===' && SCAN_TOPIC=/scan_filtered python3 auto_explorer.py" C-m
